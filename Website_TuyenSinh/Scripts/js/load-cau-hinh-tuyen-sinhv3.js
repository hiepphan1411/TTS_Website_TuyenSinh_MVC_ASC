$(function () {
  ...
  function bLabel(f) {
    if (f.maDanhMuc == "Space") {
      return (
        '<div class="col-md-' + f.colSpan + '" id="' + inputId(f) + '"></div>'
      );
    } else {
      return `
  ...
  function getInputValue($input) {
    ...
    switch (type) {
      ...
      case "numeric":
        ...
        if (numeric) {
          value = numeric.value() !== null ? String(numeric.value()) : "";
        } else {
          value = $input.val() || "";
        }
        break;
  ...
  function collectMethodDataDynamic($container) {
    ...
    if (!$body.length) {
      console.warn("[collectMethodDataDynamic] No .phuongthuc-body found");
      return methodData;
    }
    ...
    if ($wishItems.length > 0) {
      ...
      $wishItems.each(function (idx) {
        ...
        if ($scorePill.length > 0) {
          ...
          if (scoreValue && scoreValue !== "0.00 điểm" && scoreValue !== "0") {
    ...
    $allSelects.each(function (idx) {
      ...
      if ($possibleInput.length === 0) {
        ...
        $possibleInput = $(allInputs.get(idx)) || $(allInputs.get(idx + 1));
      }
    ...
    $tables.each(function () {
      ...
      $table.find("tbody tr").each(function (rowIdx) {
        ...
        $cells.each(function (colIdx) {
          ...
          if ($inputs.length > 0) {
            ...
            if ($scorePill.length) {
              ...
              if (value && value !== "0.00 điểm" && value !== "0.00") {
                hasValidData = true;
              }
            } else {
              // text cell
        ...
        if ((monName || hasValidData) && Object.keys(rowData).length > 0) {
          var allScores = [];
    ...
    $body.find(".doi-tuong-checkbox:checked").each(function () {
      ...
      if ($li.length) {
        ...
        text = $checkbox.closest(".d-flex").find(".doi-tuong-span").text();
      }
  ...
  function renderMethodDataDynamic(methodName, methodData, $methodsContainer) {
    var $methodSection = $(
        ...
        for (var label in thong_tin.fields) {
          if (!label.startsWith("_") && !shouldSkipField(label)) {
            ...
            if (value && value !== "" && value !== "-") {
              var isScore = !isNaN(parseFloat(value)) && value.trim() !== "";
              if (isScore) {
    ...
    $.each(tabs, function (tabIdx, tab) {
      ...
      if (tab.tabKey === "ThongTinXetTuyen") {
        ...
        try {
          ...
          var $mainSection = $('<div class="df-review-section"></div>');
          var $mainHeader = $('<div class="df-review-section-header"></div>');
          $mainHeader.append(
          ...
          $.each(checkedTabs, function (index, methodName) {
            console.log("[buildReviewPanel] Rendering method:", methodName);
            var methodData = tabsData[methodName] || {};
            renderMethodDataDynamic(methodName, methodData, $methodsContainer);
          });
          ...
          console.error("[buildReviewPanel] Error parsing xet tuyen data", e);
        }
