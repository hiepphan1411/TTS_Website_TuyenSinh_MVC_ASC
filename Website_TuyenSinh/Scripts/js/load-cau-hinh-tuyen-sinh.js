...
$(function() {
            ...
            function renderShell() {
                ...
                $card.append(
                    ...
                    '<button class="df-btn-back" id="df-btn-back">' +
                    '<i class="bi bi-arrow-left me-1"></i> Quay về</button>' +
                    '<button class="df-btn-next" id="df-btn-next">' +
                    'Hoàn thành &amp; tiếp tục ' +
            ...
            function bindNav() {
                ...
                $('#df-btn-next')
                    .off('click')
                    .on('click', function() {
                        var curTab = tabs[curTabIdx];
                        var isLastTab = curTabIdx === tabs.length - 1;
                        var isReviewTab = curTab && curTab.isReviewTab;

                        // Xử lý tab review
                        if (isReviewTab) {
                            var isConfirmed = $('#df-confirm-review').prop('checked');
                            if (!isConfirmed) {
                                alert('Vui lòng xác nhận thông tin trước khi hoàn tất');
                                return;
                            }
                            // Xử lý submission ở đây
                        ...
                        if (!validateTab(curTabIdx)) {
                            ...
                            if ($first.length) {
                                $('html,body').animate({ scrollTop: $first.offset().top - 90 },
                                    280,
                                );
                            }
                            return;
